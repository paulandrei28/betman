...
function renderReport(data) {
  const groups = Object.entries(data.predictions || data || {});
...
async function openReport(date, updateHistory = true) { const entry = state.reports.get(date); if (!entry) return; state.selected = date; document.body.classList.add('report-focus'); $('.workspace').classList.add('report-focus'); $('.report-panel').hidden = false; renderCalendar(); $('#report-error').hidden = true; $('#report-content').hidden = true; $('#report-loading').hidden = false; if (updateHistory) history.pushState(null, '', `#${date}`); try { const response = await fetch(entry.analysis); if (!response.ok) throw Error(); const data = await response.json(); $('#report-heading').textContent = displayDate(reportDate(date)); $('#report-body').innerHTML = renderReport(data); $('#report-content').hidden = false; } catch (error) { $('#report-error').hidden = false; } finally { $('#report-loading').hidden = true; } }
...
async function loadReports() { try { const response = await fetch('data/reports.json'); if (!response.ok) throw Error(); const payload = await response.json(); for (const entry of payload.reports || []) state.reports.set(entry.date, entry); renderCalendar(); const hash = location.hash.slice(1); if (state.reports.has(hash)) { state.month = new Date(Number(hash.slice(0, 4)), Number(hash.slice(4, 6)) - 1, 1); openReport(hash, false); } } catch (error) { $('.report-panel').hidden = false; $('#report-error').hidden = false; } }
