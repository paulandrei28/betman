...
async function openReport(date, updateHistory = true) { const entry = state.reports.get(date); if (!entry) return; state.selected = date; document.body.classList.add('report-focus'); $('.workspace').classList.add('report-focus'); $('.report-panel').hidden = false; renderCalendar(); $('#report-error').hidden = true; $('#report-content').hidden = true; $('#report-loading').hidden = false; if (updateHistory) history.pushState(null, '', `#${date}`); try { const reportUrl = new URL(entry.report || entry.analysis, document.baseURI); const response = await fetch(reportUrl, { cache: 'no-store' }); if (!response.ok) throw Error(); const data = await response.json(); $('#report-heading').textContent = displayDate(reportDate(date)); $('#report-body').innerHTML = renderReport(data); $('#report-content').hidden = false; } catch (error) { $('#report-error').hidden = false; } finally { $('#report-loading').hidden = true; } }
