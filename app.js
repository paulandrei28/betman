...
function renderEvidence(items, prediction) {
  if (!items?.length) return `<p class="muted">${t('noEvidence')}</p>`;
  return `<ul>${items.map((item) => { const team = item.team === 'home' ? prediction?.home : item.team === 'away' ? prediction?.away : null; const label = team || (item.team ? (state.lang === 'ro' ? (item.team === 'both' ? 'meciuri directe' : evidenceTranslations[item.team] || item.team) : (item.team === 'both' ? 'head-to-head' : evidenceTranslations[item.team] || item.team)) : t('directEvidence')); const name = state.lang === 'ro' ? translateMarket(item.name || '') : item.name || ''; const value = translateEvidenceText(item.value ?? '', prediction); return `<li><strong>${escapeHtml(label)}</strong>${name ? ` ${escapeHtml(name)}:` : ''} ${escapeHtml(value || '?')}</li>`; }).join('')}</ul>`;
}
